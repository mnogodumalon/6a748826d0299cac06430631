...
import type { NightcorePlaylists, CreateNightcorePlaylists } from '@/types/app';
...
export class LivingAppsService {
  // --- NIGHTCORE_PLAYLISTS ---
  static async getNightcorePlaylists(): Promise<NightcorePlaylists[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.NIGHTCORE_PLAYLISTS}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as NightcorePlaylists[];
    return enrichLookupFields(records, 'nightcore_playlists');
  }
  static async getNightcorePlaylist(id: string): Promise<NightcorePlaylists | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.NIGHTCORE_PLAYLISTS}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as NightcorePlaylists;
    return enrichLookupFields([record], 'nightcore_playlists')[0];
  }
  static async createNightcorePlaylist(fields: CreateNightcorePlaylists): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.NIGHTCORE_PLAYLISTS}/records`, { fields: cleanFieldsForApi(fields as any, 'nightcore_playlists') });
    return { ...data, record_id: data.id };
  }
  static async updateNightcorePlaylist(id: string, fields: Partial<CreateNightcorePlaylists>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.NIGHTCORE_PLAYLISTS}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'nightcore_playlists') });
    return { ...data, record_id: data.id };
  }
  static async deleteNightcorePlaylist(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.NIGHTCORE_PLAYLISTS}/records/${id}`);
  }
