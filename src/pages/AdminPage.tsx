...
import { NightcorePlaylistsViewDialog } from '@/components/dialogs/NightcorePlaylistsViewDialog';
...
const NIGHTCOREPLAYLISTS_FIELDS = [
  { key: 'titel', label: 'Playlist-Titel', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'playlist_url', label: 'Playlist-Link', type: 'string/url' },
  { key: 'cover_bild', label: 'Cover-Bild', type: 'file' },
  { key: 'genre_tags', label: 'Genre-Tags', type: 'multiplelookup/checkbox', options: [{ key: 'nightcore_classic', label: 'Nightcore Classic' }, { key: 'nightcore_anime', label: 'Nightcore Anime' }, { key: 'nightcore_gaming', label: 'Nightcore Gaming' }, { key: 'nightcore_pop', label: 'Nightcore Pop' }, { key: 'nightcore_rock', label: 'Nightcore Rock' }, { key: 'nightcore_edm', label: 'Nightcore EDM' }, { key: 'nightcore_metal', label: 'Nightcore Metal' }, { key: 'nightcore_chill', label: 'Nightcore Chill' }, { key: 'nightcore_remix', label: 'Nightcore Remix' }] },
  { key: 'anzahl_songs', label: 'Anzahl der Songs', type: 'number' },
  { key: 'erstellt_am', label: 'Erstellt am', type: 'date/date' },
  { key: 'plattform', label: 'Plattform', type: 'lookup/select', options: [{ key: 'youtube', label: 'YouTube' }, { key: 'spotify', label: 'Spotify' }, { key: 'soundcloud', label: 'SoundCloud' }, { key: 'apple_music', label: 'Apple Music' }, { key: 'andere', label: 'Andere' }] },
  { key: 'bewertung', label: 'Bewertung', type: 'lookup/radio', options: [{ key: 'stern_1', label: '⭐ 1 – Nicht so gut' }, { key: 'stern_2', label: '⭐⭐ 2 – Geht so' }, { key: 'stern_3', label: '⭐⭐⭐ 3 – Gut' }, { key: 'stern_4', label: '⭐⭐⭐⭐ 4 – Sehr gut' }, { key: 'stern_5', label: '⭐⭐⭐⭐⭐ 5 – Absoluter Favorit' }] },
  { key: 'notizen', label: 'Persönliche Notizen', type: 'string/textarea' },
  { key: 'favorit', label: 'Favorit', type: 'bool' },
];

const ENTITY_TABS = [
  { key: 'nightcore_playlists', label: 'Nightcore Playlists', pascal: 'NightcorePlaylists' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('nightcore_playlists');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'nightcore_playlists': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'nightcore_playlists': return (data as any).nightcorePlaylists as NightcorePlaylists[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'nightcore_playlists': return NIGHTCOREPLAYLISTS_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'nightcore_playlists': return {
        create: (fields: any) => LivingAppsService.createNightcorePlaylist(fields),
        update: (id: string, fields: any) => LivingAppsService.updateNightcorePlaylist(id, fields),
        remove: (id: string) => LivingAppsService.deleteNightcorePlaylist(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'nightcore_playlists' || dialogState?.entity === 'nightcore_playlists') && (
        <NightcorePlaylistsDialog
          open={createEntity === 'nightcore_playlists' || dialogState?.entity === 'nightcore_playlists'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'nightcore_playlists' ? handleUpdate : (fields: any) => handleCreate('nightcore_playlists', fields)}
          defaultValues={dialogState?.entity === 'nightcore_playlists' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['NightcorePlaylists']}
          enablePhotoLocation={AI_PHOTO_LOCATION['NightcorePlaylists']}
        />
      )}
      {viewState?.entity === 'nightcore_playlists' && (
        <NightcorePlaylistsViewDialog
          open={viewState?.entity === 'nightcore_playlists'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'nightcore_playlists', record: r }); }}
        />
      )}
